...
<script>
    ...
    export default {
        ...
        created() {
            ...
            HTTP.get('/users/'+this.id+'/upvoted_submissions',{headers: {'Authorization': localStorage['googleId']}}).then(response => {
        ...
        methods: {
            ...
            remove(contribution) {
                HTTP.delete('/posts/'+contribution.id,{headers: {'Authorization': localStorage['googleId']}}).then(() => {
                    HTTP.get('/users/'+localStorage['userId']+'/upvoted_submissions',{headers: {'Authorization': localStorage['googleId']}}).then(response => {
                        this.contributions = response.data
                    }).catch(e => {
                        this.errors.push(e);
                    });
